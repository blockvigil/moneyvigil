<script>
	...
	async function invite(){
		inviteEnabled = false;
		var d = await makeAuthenticateCall('/inviteuser', {
			name: inviteName,
			email: inviteEmail
		});
		if (!d.success){
			...
			if (d.message == "InviteExists"){
				M.toast({html: 'User has already been invited!'});
				dispatch("close");
				return;
			}
		...
		if (d.invitedStatus){
			if (d.emailDeliveryStatus){
				M.toast({html: 'Invited Friend! '+d.remainingInvites+' remaining invite'+(d.remainingInvites == 1 ? '': 's')+'.'});
			} else {
				M.toast({html: 'Invited Friend! We will send an email shortly. '+d.remainingInvites+' remaining invite'+(d.remainingInvites == 1 ? '': 's')+'.'});
			}
		} else {
			M.toast({html: 'You are out of invites but you may still add your friend on bills. We will email them as we release more!'});
		}
		dispatch("close");
	}
</script>
<Modal disableOk={true} on:close="{() => dispatch("close")}">
	...
	<div class="row">
		<div class="input-field col s12">
			...
			<label for="inviteName">Friend's Name</label>
		</div>
		<div class="input-field col s12">
